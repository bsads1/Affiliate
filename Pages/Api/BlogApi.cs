...
public class BlogApi : IRoute
{
    public void Map(WebApplication app)
    {
        app.MapDelete("/api/blog", async (HttpContext context, PageService pageService) =>
            {
                try
                {
        ...
        app.MapPost("/api/blog",
                async ([FromForm] PagePostFormDto form, HttpContext context, PageService pageService) =>
                {
                    ...
                    var result = await pageService.CreateBlogAsync(form);
        ...
        app.MapPut("/api/blog",
                async ([FromForm] PagePostFormDto form, HttpContext context, PageService pageService) =>
                {
                    if (form.Id > 0)
                    {
                        ...
                        var result = await pageService.UpdateBlogAsync(form);
                        if (result != null)
                        {
                            return Results.Json(new
                            {
